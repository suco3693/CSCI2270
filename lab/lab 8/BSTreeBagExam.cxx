...
int test3( )
{
    BSTreeBag<int> test;
    unsigned int items[4] = { 0, 0, 0, 0 };
    char *oldbytes = new char[sizeof(BSTreeBag<int>)];
    char *newbytes = new char[sizeof(BSTreeBag<int>)];
    unsigned int i;
    ...
    cout << flush;
    memcpy(oldbytes, &test, sizeof(BSTreeBag<int>));
    test = test;
    memcpy(newbytes, &test, sizeof(BSTreeBag<int>));
    for (i=0; i < sizeof(BSTreeBag<int>); i++)
...
int test5( )
{
	...
	BSTreeBag<int> test3 = test1 + test2;
	
	unsigned int arr[100];
	for (unsigned int k = 0; k < 100; ++k)
		arr[k] = test3.count(k);
	if (arr[98] + arr[97] + arr[99] + arr[3] + arr[7] + arr[14] + arr[20] + arr[6] + arr[19] + arr[5] != test3.size())	
		cout << "operator + failed" << endl;
	test2 += test1;
	for (unsigned int k = 0; k < 100; ++k)
		arr[k] = test2.count(k);
	if (arr[98] + arr[97] + arr[99] + arr[3] + arr[7] + arr[14] + arr[20] + arr[6] + arr[19] + arr[5] != test2.size())	
		cout << "operator += failed" << endl;
	test1 += test1;
	for (unsigned int k = 0; k < 100; ++k)
		arr[k] = test1.count(k);
