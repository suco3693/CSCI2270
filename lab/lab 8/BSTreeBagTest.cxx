...
#include "BSTreeBag.h"     // Provides the BSTreeBag<double> class (with Item as a double)
using namespace std;
...
void display_BSTreeBags(const BSTreeBag<double>& b1, const BSTreeBag<double>& b2);
...
BSTreeBag<double> copyBSTreeBag(BSTreeBag<double> b);
...
int main( )
{
    BSTreeBag<double> b1, b2;  // Bags that we'll perform tests on
    ...
    do
    {
	    ...
	    case 'a': b2 = b1;
		      break;
            case 'C': b1 = copyBSTreeBag(b2);
                      break;
            case 'c': b2 = copyBSTreeBag(b1);
                      break;
            case 'S':
            case 's': cout << "The BSTreeBags' sizes are " << b1.size( );
            ...
            case 'e': number = get_number();
                      ...
                      break;
            case 'D':
            case 'd': display_BSTreeBags(b1, b2);
                      break;
	    case 'q':
