...
int main()
	{
		...
		v = aBag.toVector();
		if (v.size() > 0);
			cout << "The Vector is greater than zero: that is good" << endl;
		// Find is the Bag Size is greater than ten
		if (aBag.getCapacity() > 10)
			cout << "The Bag Size is greater than ten: Good to know" << endl;
		else
			cout << "The Bag Size is less than ten: Good to know" << endl;
		// If there are no tortas remove a taco
		aBag.add("taco");
		aBag.add("taco");
		//aBag.add("torta");
		if (aBag.getFrequencyOf("torta") == 0)
		{
			while (aBag.getFrequencyOf("taco") > 0)
			{
				aBag.remove("taco");
			}	
			cout << "There are no tortas so we can not have tacos" << endl;
		}
		else 
			cout << "There are tortas so we can have tacos" << endl;
